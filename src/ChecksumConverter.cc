...
{
printf("crc_0x8005 with len = %d, data = %s.\n", len, data);
...
static ulong crc_0x1021(const uchar* data, ulong len, ulong crc)
{
    ...
    while (len--) crc = table[(crc>>8) & 0xff] ^ (crc << 8 ^ *data++);
    return crc;
}
/*  --- table for calculation of PTW CRC checksum ---  */
unsigned short const crctab[256]  =  {
        0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7,
        0x8108, 0x9129, 0xA14A, 0xB16B, 0xC18C, 0xD1AD, 0xE1CE, 0xF1EF,
        0x1231, 0x0210, 0x3273, 0x2252, 0x52B5, 0x4294, 0x72F7, 0x62D6,
        0x9339, 0x8318, 0xB37B, 0xA35A, 0xD3BD, 0xC39C, 0xF3FF, 0xE3DE,
...
unsigned short BCSupdate( unsigned short bcs, unsigned char c)
{
    unsigned short  wRes;
    wRes = crctab[(bcs>>8) & 0xff] ;
    wRes ^= ((bcs<<8) ^ c);
    return( wRes );
}

unsigned short BCScalc( unsigned char *ptr, unsigned char *ende)
{
    register unsigned short newbcs = 0;

    while ( ptr < ende )
    {
        newbcs = BCSupdate( newbcs, *ptr++);
    }

    return( newbcs);
}

unsigned short chksumCalc( unsigned char *ptr, unsigned char *ende)
{
    register unsigned short newbcs;

    newbcs = BCScalc( ptr, ende );
    newbcs = BCSupdate( newbcs, 0);
    newbcs = BCSupdate( newbcs, 0);
    return( newbcs);
}

static ulong crc_PTW(const uchar* data, ulong len, ulong crc)
{
   unsigned char *pEnd;        /* pointer just before CRC checksum */
   unsigned short chkSum;      /* temporary storage of checksum value */
   unsigned char *localData;

   localData = (unsigned char *)data;
   /* Point to last position */
   pEnd = localData + len;
   chkSum = chksumCalc(localData, pEnd);
   crc = (ulong)chkSum;
   return crc;
}
...
//    name      func              init        xorout  bytes  chk("123456789")
    ...
    {"unidos",  crc_PTW,          0x0000,     0x0000,     0}  // unidos
};

static ulong mask[5] = {0xFFFFFF, 0xFF, 0xFFFF, 0xFFFFFF, 0xFFFFFFFF};
...
printPseudo(const StreamFormat& format, StreamBuffer& output)
{
    ...
    {
      // TL added CRC for PTW unidos device
      if(checksumMap[fnum].bytes == 0)
      {
          ulong tmp;
          ulong val=sum;
          for(i = 0; i<5; i++)
          {
             sum = val;
             tmp = sum % 10;  // only take last digit
             tmpchar[i] = tmp + '0';
             val = val/10;
          }
          for(i=0; i<5; i++)
             output.printf("%c", tmpchar[4-i]);
      }
      else  // execute other checksum code
      {
